...
export function setVariable(data, key, value, convertKeyToPath = false) {
  data = data || {};
...
export const tokenize = (str, data, defaultFilter = "| html") => {
...
export const filterDate = (value, data = {}, format = "X", utc = false) => {
  let m,
...
export function parseDuration(str) {
  ...
  if (matches) {
    const duration = moment.duration(parseFloat(matches[1]), matches[2]);
...
export function resolveVariable(path, data = {}) {
  if (path === "&" || path == "$$") {
    return data;
...
export function isPureVariable(path) {
