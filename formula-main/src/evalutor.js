...
export class Evaluator {
  ...
  nsVariable(ast) {
    if (ast.namespace === "window") {
      this.contextStack.push((name) => (name === "&" ? window : window[name]));
   ...
   * @returns
   */
  callAnonymousFunction(ast, args) {
    ...
    this.contextStack.push((varName) => (varName === "&" ? ctx : ctx[varName]));
   ...
   * @returns {Date} 日期对象
   */
  fnDATE(year, month, day, hour, minute, second) {
    if (month === undefined) {
   ...
   */
  fnDATETOSTR(date, format = "YYYY-MM-DD HH:mm:ss") {
